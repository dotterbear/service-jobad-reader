...
public class JobAdService {
	...
	public Page<JobAd> findAll(int page, int size, String direction, String orderBy) {
		log.debug("findAllOrderByTs, page: {}, size: {}, direction: {}, orderby: {}", page, size, direction, orderBy);
		return jobAdRepository.findAll(PageRequest.of(page - 1, size, Sort.by(buildSortDirection(direction), orderBy)));
	}

	public Page<JobAd> searchByQuery(int page, int size, String direction, String orderBy, String query) {
		log.debug("findAllOrderByTs, page: {}, size: {}, direction: {}, orderby: {}, query: {}", page, size, direction,
				orderBy, query);
		Pageable pagable = PageRequest.of(page - 1, size, Sort.by(direction, orderBy));
		Query dbQuery = TextQuery.queryText(TextCriteria.forDefaultLanguage().matchingAny(query)).with(pagable);
