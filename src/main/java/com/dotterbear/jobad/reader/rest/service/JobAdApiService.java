...
public class JobAdApiService {
  ...
  private static JobAdItem buildJobAdItem(JobAd jobAd) {
    if (jobAd == null)
      return null;
    return new JobAdItem().id(jobAd.getId()).companyName(jobAd.getCompanyName())
        .title(jobAd.getTitle()).industry(jobAd.getIndustry()).careerLevel(jobAd.getCareerLevel())
        .qualification(jobAd.getQualification()).location(jobAd.getLocation())
        .employmentType(jobAd.getEmploymentType()).others(jobAd.getOthers())
        ...
        .fromWebSite(jobAd.getFromWebSite().name()).url(jobAd.getUrl()).score(jobAd.getScore());
  }
}
